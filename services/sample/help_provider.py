...
class ModuleHelpProvider(ModuleHelpProvider):
    """模块帮助信息提供者"""
    ...
    def get_basic_commands(self) -> str:
        ...
        return (
            f"• /{command_names['add']} <链接> <频道ID> - 添加账号订阅\n"
            f"• /{command_names['del']} <链接> <频道ID> - 删除账号订阅\n"
    ...
    def get_debug_commands(self) -> str:
        ...
        command_names = get_command_names()
        return (
            f"• /{command_names['debug_show']} <链接> - 显示单个内容项"
        )
    ...
    def get_examples(self) -> str:
        ...
        command_names = get_command_names()
        return (
            f"• /{command_names['add']} https://www.example.com/user/MS4wLjABAAAA... @mychannel\n"
            f"• /{command_names['add']} https://v.example.com/iM5g7LsM/ -1001234567890\n"
            f"• /{command_names['list']} @mychannel\n"
            f"• /{command_names['del']} https://www.example.com/user/MS4wLjABAAAA... @mychannel"
        )

    def get_description(self) -> str:
        """获取模块功能描述"""
        return "   • 支持账号订阅，自动推送最新视频和动态"
