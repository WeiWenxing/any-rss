...
class DouyinSender:
    ...
    async def _send_video_content(self, bot: Bot, content_info: dict, caption: str, target_chat_id: str) -> None:
        ...
        # 尝试每个URL直到成功
        for i, (url_type, video_url) in enumerate(video_urls):
            try:
                logging.info(f"尝试发送视频 ({url_type}): {video_url[:100]}...")
                ...
                try:
                    ...
                    media_group = [
                        InputMediaVideo(
                            ...
                            caption=caption if i == 0 else None,  # 只在第一次尝试时添加caption
                            parse_mode='Markdown'
                        )
                    ]
                    ...
                    logging.info(f"视频发送成功 ({url_type}): {content_info.get('title', '无标题')}")
                    return  # 成功发送，退出循环
                    
                except TelegramError as telegram_error:
                    logging.warning(f"Telegram发送失败 ({url_type}): {str(telegram_error)}")
                    continue
        ...
        # 所有URL都失败了
        raise Exception(f"所有视频URL都发送失败，尝试了 {len(video_urls)} 个URL")
