...
│       └── latest.json             # 最新内容引用（与douyin模块保持一致，减少API调用）
└── media/                          # 媒体文件目录
    └── {url_hash}/                 # 按URL哈希分组
...
```json
{
  "title": "GitHub Issue: RSSHub - 修复B站视频解析问题",
  "link": "https://github.com/DIYgod/RSSHub/issues/1234",
  "author": "contributor123",
  "published": "2024-05-06T12:34:56Z",
  "item_id": "entry_abc123def456",
  "description": "修复了B站视频无法正确解析的问题..."
}
```
...
**latest.json缓存机制**：
- 每次检查更新时，将最新的内容项保存为`latest.json`
- 作用：无需重新调用API即可获取最新内容的基本信息（标题、作者、发布时间等）
- 场景：在列表命令中展示最新内容信息，提升用户体验
- 实现：在`check_updates`方法中，当发现新内容时保存最新条目为`latest.json`
- 与douyin模块保持一致：共享相同的数据结构和存储逻辑

**多频道转发优化**：
- 每个新RSS条目仅需1次发送 + N-1次转发，而非N次发送
- 使用`copy_messages`方法实现内容一致的无痕转发
- 在网络带宽、API调用次数和服务器资源方面均有显著节省

#### 9.1.2 存储优化

**精简的存储结构**：
- 只保存必要的索引文件（`known_item_ids.json`）和最新引用（`latest.json`）
- 不持久化存储完整的RSS条目内容，减少存储空间占用
- 媒体文件按需下载，不必要时不保存
...
本设计文档详细描述了RSSHub模块的架构、数据结构和关键算法，特别是如何实现高效的多频道推送机制。通过复用douyin模块的成熟转发机制，并结合RSS模块的媒体处理策略，RSSHub模块能够提供高效、可靠的RSS订阅服务。

**关键技术亮点**：
1. **多频道高效转发**: 每个新RSS条目仅需1次发送 + N-1次转发
2. **智能历史对齐**: 新频道订阅时自动同步历史RSS条目
3. **容错降级机制**: 多层次的失败恢复策略